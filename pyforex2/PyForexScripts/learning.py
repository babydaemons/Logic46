...
def execute(contents, predict_columns:int, columns:int):
    ...
    similarity = cosine_similarity(y, y_predict)
    result = { "ReconstructionError": str(reconstruction_error), "MeanError": str(mean_error), "Similarity": str(similarity) }
    print(f"再構築誤差: {reconstruction_error}, 誤差平均: {mean_error}, 類似度: {similarity}") 
    print(f"{BEGIN1}⇒  完了{END}")
