...
enum ENUM_TRAILING_TYPE {
    TRAILING_TYPE_WITHOUT_TP, // TP設定なし要裁量決済
    TRAILING_TYPE_WITH_TP_SL, // TP/SLトレイル幅個別設定
    TRAILING_TYPE_WITH_TP, // TP固定SLのみトレイル
};
...
input double STOP_LOSS = 2000;                                      // ├損切価格幅
sinput int SLIPPAGE = 100;                                          // └スリッページ(ポイント)
sinput string DUMMY10 = "";                                         // トレーリングストップ設定
sinput bool TRAILING_STOP_ENABLE = true;                            // ├有効/無効
sinput ENUM_TRAILING_TYPE TRAILING_TYPE = TRAILING_TYPE_WITH_TP_SL; // ├トレーリングストップ種別
sinput int TRAILING_STOP_INTERVAL_SECONDS = 13;                     // ├トレーリングストップ更新間隔(秒)
input double TRAILING_TAKE_PROFIT = 600;                            // ├トレーリングストップ利確価格幅
input double TRAILING_STOP_LOSS = 1200;                             // └トレーリングストップ損切価格幅
