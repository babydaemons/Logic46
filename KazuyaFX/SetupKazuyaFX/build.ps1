...
try {
    ...
    if ($chocoVersion) {
        ...
        Write-Host "!!!! Chocolatey が見つかりません。インストールを試みます..." -ForegroundColor Red
        if (Test-Path "C:\ProgramData\chocolatey") {
            Write-Host "!!!! 既存の Chocolatey インストールが破損している可能性があります。" -ForegroundColor Red
    ...
    # === EXE を実行 ===
    Write-Host "`n`n`n>>>>>>>>>>>>>>>>>>>> SetupKazuyaFX.exe を実行します >>>>>>>>>>>>>>>>>>>>`n" -ForegroundColor Yellow
