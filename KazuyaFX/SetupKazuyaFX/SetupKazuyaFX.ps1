...
try {
    ...
    if ($portCheck) {
        Write-Host "⚠️ ポート80を使用しているアプリがあります。停止を試みます..." -ForegroundColor Red
    ...
    # === NGINX のインストール ===
    Write-Host "🛠️ NGINX（ウェブサーバー）をインストールしています..." -ForegroundColor Yellow
    ...
    # === NGINX サービスとして登録 ===
    Write-Host "⚙️ NGINX を Windows サービスとして登録しています..." -ForegroundColor Yellow
