...
/// </summary>
/// <remarks>
/// GET /api/student?name={name}&entry={entry}&buy={buy}&symbol={symbol}&lots={lots}&command={command}&ticket={ticket}
/// </remarks>
app.MapGet("/api/student", ([FromServices] PositionDao positionDao, HttpContext context) =>
{
    lock (positionDao)
    {
        ...
        var positionId = $"{position.name}-{position.ticket}";
        if (position.entry == 1)
        {
...
app.MapGet("/api/teacher", (HttpContext context, PositionDao positionDao) =>
{
    ...
    try
    {
        while (positionDao.GetPosition(out var position))
        {
            var entry = position.entry == +1 ? "Entry" : "Exit";
            var buy = position.buy == +1 ? "Buy" : "Sell";
            var line = $"\"{position.name}\",\"{position.entry}\",\"{position.buy}\",\"{position.symbol}\",\"{position.lots}\",\"{position.ticket}\"\n";
            lines += line;
            var Entry = position.entry == +1 ? "[Entry]," : "[Exit], ";
            var Buy = position.buy == +1 ? "[Buy], " : "[Sell],";
            string message = $"生徒さん[{position.name}], 売買{Entry} ポジション{Buy} 通貨ペア[{position.symbol}], 売買ロット[{position.lots:F2}], 売買番号[ {position.ticket}]";
